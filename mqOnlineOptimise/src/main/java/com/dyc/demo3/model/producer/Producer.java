        ...
        if( null == producer ){
            synchronized ( Producer.class ){
                if( producer == null){
                    ...
                    for( int i = 0; i < QueueRunnable.initNum; i++){
                        QueueRunnable.executorService.execute( new QueueRunnable().new PlayerDaoRunnable(new ArrayList<Player>()) );
                    }
                }
            }
        }
